<template>
  ...
  >
    ...
    <div v-show="store.states.columns.length === 0" class="ww-empty-columns">
      暂无表头
    </div>
    <dropdown
      :columns-width="columnsWidth"
    ...
    <scroll bar-type="y" :store="store"> </scroll>
  </div>
</template>
    ...
    multiSelectAdjustPostion(e) {
      ...
      if (states.selector.isSelected || states.autofill.isAutofill) {
        ...
        const sBottom =
          ...
          10;
        const sRight =
          this.excelPos.left + this.$refs.willtable.offsetWidth - 10;
        let { tableBodyTop } = states;
        let { tableBodyLeft } = states;
