<template>
	...
	>
		...
		>
			...
			<table-body
				ref="tbody"
				:rowFixedNum="rowFixedNum"
				:columnsWidth="columnsWidth"
				:all-show="true"
				:rowFixedData="rowFixedData"
				:cellStyle="cellStyle"
		...
		></div>
		<scroll v-if="tableWidth > wrapperWidth" barType="x" :store="store" />
		<scroll barType="y" :store="store" />
	</div>
</template>
...
export default {
	...
	props: {
		...
		pageSize: {
			// 分页器每页数量,加.sync后可自动更新变量的值,改变后会触发pageChange事件
			type: Number,
			default: 20,
		},
		pageNum: {
			// 分页器当前第几页,加.sync后可自动更新变量的值,改变后会触发pageChange事件
			type: Number,
			default: 1,
		},
		total: {
	...
	computed: {
		page: {
			...
			set(val) {
				...
				return this.$emit('update:pageNum', val);
			},
		},
		size: {
			get() {
	...
	methods: {
		...
		initData() {
			...
			this.rowFixedData = this.data.slice(0, this.rowFixedNum); // 需要固定的行的数据
		...
		handleResize() {
			...
			this.$nextTick(() => {
				...
				if (this.allSetWidth) {
					states.tableWidth = states.columns
						.map((cell) => cell.width)
						.reduce((a, b) => a + b, 0);
				} else {
					states.tableWidth = this.columnsWidth.reduce(
						(total, cur) => total + cur,
						0
					);
		...
		getContentToclipboard() {
			...
			for (
				let i = 0;
				i <= states.selector.selectedYArr[1] - states.selector.selectedYArr[0];
				i += 1
			) {
				for (
					let j = 0;
					j <=
					states.selector.selectedXArr[1] - states.selector.selectedXArr[0];
					j += 1
				) {
					if (
						states.selector.selectedXArr[1] -
							states.selector.selectedXArr[0] ===
						0
					) {
						content += `${
							states.showData[i + states.selector.selectedYArr[0]][
		...
		clipboardToContent(e) {
			setTimeout(() => {
				...
				const selectorXLength =
					states.selector.selectedXArr[1] - states.selector.selectedXArr[0] + 1;
				const selectorYLength =
					states.selector.selectedYArr[1] - states.selector.selectedYArr[0] + 1;
				if (
					selectorYLength % arr.length === 0 &&
					selectorXLength % arr[0].length === 0
				) {
					for (let i = 0; i <= selectorYLength - 1; i += 1) {
						for (let j = 0; j <= selectorXLength - 1; j += 1) {
							if (
								!states.columns[j + states.selector.selectedXArr[0]].disabled
							) {
								states.showData[i + states.selector.selectedYArr[0]][
									states.columns[j + states.selector.selectedXArr[0]].key
								] = arr[i % arr.length][j % arr[0].length];
							}
						}
					}
				} else {
					for (let i = 0; i <= arr.length - 1; i += 1) {
						for (let j = 0; j <= arr[i].length - 1; j += 1) {
							if (
								states.showData[i + states.selector.selectedYArr[0]] &&
								states.columns[j + states.selector.selectedXArr[0]]
							) {
								if (
									!states.columns[j + states.selector.selectedXArr[0]].disabled
								) {
		...
		setEditing(key) {
			...
			states.editor.editType = states.columns[states.editor.editorXIndex].type
				? states.columns[states.editor.editorXIndex].type
				: 'text';
			states.editor.curEditorWidth =
				this.columnsWidth[states.editor.editorXIndex];
			if (
				key &&
				(states.editor.editType === 'text' ||
					states.editor.editType === 'number')
			) {
		...
		adjustPosition() {
			...
			if (this.fixedCount > 0) {
				const unFixedLeftArr = this.scrollLeftArr
					.slice(this.fixedCount - 1, -1)
					.map((item) => item - this.fixedWidth);
				const unFixedRightArr = this.scrollLeftArr
					.slice(this.fixedCount)
					.map((item) => item - this.fixedWidth);
				curLeftShould =
					unFixedLeftArr[states.editor.editorXIndex - this.fixedCount];
				curRightShould =
					unFixedRightArr[states.editor.editorXIndex - this.fixedCount] +
					this.fixedWidth -
					this.wrapperWidth +
					states.scrollBarWidth +
					1;
				...
				curRightShould =
					scrollLeftArr[states.editor.editorXIndex + 1] -
					this.wrapperWidth +
					states.scrollBarWidth +
					1;
			}
			if (states.tableWidth > this.wrapperWidth) {
				...
				states.scrollbar.posX =
					(states.tableBodyLeft / (states.tableWidth - states.mainWidth)) *
					(states.mainWidth - states.scrollbar.xWidth);
			}
			// 上下调整
			if (this.maxHeight) {
				...
				const curBottomShould =
					this.scrollTopArr[states.editor.editorYIndex + 1] -
					this.maxHeight +
					1;
				if (states.tableBodyTop < curBottomShould) {
					states.tableBodyTop = curBottomShould;
				}
				states.scrollbar.posY =
					(states.tableBodyTop / (states.tableHeight - states.mainHeight)) *
					(states.mainHeight - states.scrollbar.yHeight);
			}
		},
		multiSelectAdjustPostion(e) {
			...
			if (states.selector.isSelected || states.autofill.isAutofill) {
				...
				if (
					states.selector.selectedYArr[0] === states.selector.selectedYArr[1] &&
					states.selector.selectedXArr[0] === states.selector.selectedXArr[1]
				) {
					return;
				}
				const sTop =
