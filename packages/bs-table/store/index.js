...
class TableStore {
  ...
  calcDomData() {
    const { states } = this;
    states.visibleRowStartIndex = Math.floor(
      states.tableBodyTop / states.rowHeight
    );
    states.visibleRowEndIndex =
      states.visibleRowStartIndex +
      Math.ceil(states.mainHeight / states.rowHeight);
    states.domData = states.showData.slice(
      states.visibleRowStartIndex,
      states.visibleRowEndIndex + 1
    );
  }
  ...
  autofillUp() {
    ...
    if (states.autofill.autofillYArr[1] > states.selector.selectedYArr[1]) {
      for (
        let i = 0;
        i <= states.autofill.autofillYArr[1] - states.autofill.autofillYArr[0];
        i += 1
      ) {
        for (
          let j = 0;
      ...
      states.selector.selectedYArr.splice(
        1,
        1,
        states.autofill.autofillYArr[1]
      );
    ...
    if (states.autofill.autofillYArr[0] < states.selector.selectedYArr[0]) {
        ...
        for (
          ...
          if (!states.columns[j + states.selector.selectedXArr[0]].disabled) {
            states.showData[i + states.autofill.autofillYArr[0]][
              ...
              states.showData[states.selector.selectedYArr[0]][
                states.columns[j + states.selector.selectedXArr[0]].key
              ];
          }
        }
      ...
      states.selector.selectedYArr.splice(
        0,
        1,
        states.autofill.autofillYArr[0]
      );
    }
    setTimeout(() => {
  ...
  multiSelect(e, x, y, columnType) {
    ...
    if (states.selector.isSelected) {
      setTimeout(() => {
        states.autofill.autofillXIndex =
          x > states.editor.editorXIndex ? x : states.editor.editorXIndex;
        states.autofill.autofillYIndex =
          y > states.editor.editorYIndex ? y : states.editor.editorYIndex;
        ...
        if (states.selector.selectedXIndex > states.editor.editorXIndex) {
          ...
          states.selector.selectedXArr.splice(
            1,
            1,
            states.selector.selectedXIndex
          );
        } else {
          states.selector.selectedXArr.splice(
            0,
        ...
        if (states.selector.selectedYIndex > states.editor.editorYIndex) {
          ...
          states.selector.selectedYArr.splice(
            1,
            1,
            states.selector.selectedYIndex
          );
        } else {
          states.selector.selectedYArr.splice(
            0,
  ...
  selectAllCells() {
    ...
    states.selector.selectedXArr = [
      states.editor.editorRange.minX,
      states.editor.editorRange.maxX,
    ];
    states.selector.selectedYArr = [
      states.editor.editorRange.minY,
      states.editor.editorRange.maxY,
    ];
  }
  ...
  sort(type) {
    ...
    if (type === 'ascending') {
      states.showData.sort((x, y) =>
        x[states.columnsStatusList[states.dropdown.index].key] >
        y[states.columnsStatusList[states.dropdown.index].key]
          ? 1
          : -1
      );
    } else {
  ...
  resetFilter() {
    ...
    Object.keys(states.columnsStatusList[states.dropdown.index].list).forEach(
      (key) => {
        states.columnsStatusList[states.dropdown.index].list[
          key
        ].checked = false;
      }
    );
  ...
  filterData() {
    ...
    Object.keys(states.filters).forEach((key) => {
      states.showData = states.showData.filter((item) =>
        states.filters[key].includes(item[key].toString())
      );
    });
    states.dropdown.index = null;
  }
  ...
  operation(type) {
    ...
    if (!states.editor.editing) {
      if (type === 'undo' && states.curHistory > 1) {
        states.curHistory -= 1;
      }
      if (
        type === 'recovery' &&
        states.curHistory < states.historyData.length
      ) {
        states.curHistory += 1;
      }
      states.isOperation = true;
      JSON.parse(states.historyData[states.curHistory - 1]).forEach(
        (i, index) => {
  ...
  clearSelected() {
    ...
    for (
      ...
      for (
        let j = 0;
        j <= states.selector.selectedXArr[1] - states.selector.selectedXArr[0];
        j += 1
      ) {
        if (!states.columns[j + states.selector.selectedXArr[0]].disabled) {
